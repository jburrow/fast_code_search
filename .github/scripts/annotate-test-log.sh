...
while IFS= read -r line; do
  found=1
  printf '::error::%s\n' "$line"
done < <(grep -E "panicked at|assertion .* failed|^  left:|^  right:|didn't exit successfully|signal: |SIG[A-Z]+|error: test failed|error\[E[0-9]+\]|^error: could not compile" "$log" | head -40)
if [ "$found" -eq 0 ]; then
