...
impl MappedFile {
    ...
    pub fn as_str(&self) -> Result<&str> {
        ...
        let transcoded =
            self.transcoded
                .get_or_init(|| match crate::utils::transcode_to_utf8(&self.mmap) {
                    Ok(Some(result)) => {
    ...
    pub fn add_file(&mut self, path: impl AsRef<Path>) -> Result<u32> {
        ...
        let canonical = match path.canonicalize() {
            ...
            Err(e) => {
                ...
                warn!(
                    "Failed to canonicalize path '{}': {}. This may be due to broken symlinks, permission issues, or non-existent files. Using original path.",
                    path.display(),
                    e
                );
                path.to_path_buf()
